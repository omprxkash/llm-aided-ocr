...
async def main():
    try:
        ...
        input_pdf_file_path = 'testnew.pdf'
        max_test_pages = 0
        ...
        if quality_score is not None:
            ...
            #engine.save_to_file(str(quality_score),'qualityscore.wav')
            #engine.save_to_file(str(explanation),'explanation.wav')
        else:
            logging.warning("Unable to determine final quality score.")
            #engine.save_to_file('Unable to determine final quality score.','unable1.wav')
    except Exception as e:
        logging.error(f"An error occurred in the main function: {e}")
        #engine.save_to_file('error','error.wav')
...
def index():
    ...
    if request.method == 'POST':
        ...
        original_text = get_llm_response(f"Clean this OCR text:\n\n{raw_text}")
        summary = get_llm_response(f"Summarize this text:\n\n{original_text}")
        translation = get_llm_response(f"Translate this text to French:\n\n{original_text}")
        entities = get_llm_response(f"Extract named entities from the text:\n\n{original_text}")

        audio_files = {
            'original': generate_audio(original_text, 'original.mp3'),
            'summary': generate_audio(summary, 'summary.mp3'),
            'translation': generate_audio(translation, 'translation.mp3'),
            'entities': generate_audio(entities, 'entities.mp3')
        }
